...
what_is_staking = dcc.Markdown(
    ...
    stakers is a powerful tool used to maintain system stability and manage inflation of the KLIMA supply.
    '''
)
why_should_i_stake_intro = dcc.Markdown(
    ...
    Click for an overview on why staking is important.
    '''
)
why_should_i_stake = dcc.Markdown(
    ...
    1. Enabling economic activity within the Klima ecosystem which, increases use cases for KLIMA and creates
...
how_can_i_stake = dcc.Markdown(
    ...
    KlimaDAO occur on the Polygon network and you need to have KLIMA in your wallet before you can stake.
    ...
    4. While on the Stake tab, designate the amount of KLIMA you would like to stake, click "Stake". If this is your first
    time staking KLIMA you will have two transactions. One is to approve the contract, the second is for the staking transaction.
    '''
)
staking_dynamics_intro = dcc.Markdown(
    '''
    Staking is the main strategy for participating in KlimaDAO. Staking benefits both you, the participant,
    and the protocol.

    Click for an overview of the game theory of staking.
    '''
)
...
- Buy and stake KLIMA when its price is increasing
...
Simply put, (3,3) means If everyone stakes, you and everyone other particpant benefits.
...
Over time demand for carbon reduction increases and carbon offsets become more scarce,
this creates buy pressure on an increasingly scarce asset.

If you know the most basic laws of supply and demand, this equates to an increase in the price of carbon, thus the
price of BCT. The result of this dynamic is an increase in value of the asset that backs KLIMA,
giving KLIMA a higher intrinsic value (IV).

This leads us to another important variable, intrinsic value (IV). Increasing IV creates a
loose floor price of KLIMA. Sellers are selling at a great loss if they sell below IV because the backing assets
are worth more than what they are selling for. Klima stakers have a guarantee price will return to above IV because
the protocol can use reserves to buy below IV to return the price of KLIMA to its IV.
...
allows KLIMA to grow its Protocol Owned Liquidity (POL) and treasury.
