            ...
            dbc.Row([
                ...
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            html.Div(style={'color': 'green', 'fontSize': 50}, id='rewardsKLIMA')
                        ])
                    ])
                ], width=3),
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader('Days until desired daily staking rewards', style={'color': 'white', 'fontSize': 15}),
                        dbc.CardBody([
                            html.Div(style={'color': 'green', 'fontSize': 50}, id='rewardsDaily')
                        ])
                    ])
                ], width=3),
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader('Days until desired weekly staking rewards', style={'color': 'white', 'fontSize': 15}),
                        dbc.CardBody([
                            html.Div(style={'color': 'green', 'fontSize': 50}, id='rewardsWeekly')
                        ])
                    ])
                ], width=3),
                dbc.Col([dbc.Card([
...
def klimaGrowth_Projection(growthDays,initialKlima,currentAPY,percentSale, sellDays, klimaPrice_DCA, valBuy, buyDays, priceKlima, priceofETH):
    ...
    klimaGrowth_Chart.add_trace(go.Scatter(x=klimaGrowth_df.Days, y=klimaGrowth_df.Total_klimas, name='(3,3) ROI', fill = None))
    klimaGrowth_Chart.add_trace(go.Scatter(x=klimaGrowth_df.Days, y=klimaGrowth_df.Profit_Adjusted_Total_klimas, name='(3,3) Profit adjusted ROI  ', fill=None))
    klimaGrowth_Chart.add_trace(go.Scatter(x=klimaGrowth_df.Days, y=klimaGrowth_df.DCA_Adjusted_Total_klimas, name='(3,3) DCA adjusted ROI  ', fill=None))
    klimaGrowth_Chart.add_trace(go.Scatter(x=klimaGrowth_df.Days, y=klimaGrowth_df.Min_klimaGrowth, name='Min Growth Rate  ', fill=None))
    klimaGrowth_Chart.add_trace(go.Scatter(x=klimaGrowth_df.Days, y=klimaGrowth_df.Max_klimaGrowth, name='Max Growth Rate  ', fill=None))
    ...
    klimaGrowth_Chart.update_layout(legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01,),xaxis_title="Days", yaxis_title="Total klimas")
    klimaGrowth_Chart.update_xaxes(showline=True, linewidth=0.1, linecolor='#31333F',color='white', showgrid=False,gridwidth=0.01, mirror=True)
    klimaGrowth_Chart.update_yaxes(showline=True, linewidth=0.1, linecolor='#31333F',color='white', showgrid=False,gridwidth=0.01, mirror=True, zeroline=False)
