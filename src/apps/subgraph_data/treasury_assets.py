from src.apps.subgraph_data.protocol_metrics import get_bct_treasury, get_df_for_treasury_asset_by_address

from src.apps.util.constants import BCT_ERC20_CONTRACT, BCT_USDC_PAIR, \
     KLIMA_BCT_PAIR, KLIMA_MCO2_PAIR, KLIMA_USDC_PAIR, MCO2_ERC20_CONTRACT, \
     NBO_ERC20_CONTRACT, NCT_ERC20_CONTRACT, UBO_ERC20_CONTRACT, USDC_ERC20_CONTRACT
    ...
    def create_scatters(self):
        ...
        for single_asset_data in self.asset_data:
            if self.mode == MODE_ASSET_VALUE_IN_USD:
                scatter = self.create_scatter(single_asset_data, single_asset_data.formatted_market_value)
                scatters.append(scatter)
            elif self.mode == MODE_CARBON_CUSTODIED:
                if single_asset_data.custodiesCarbon:
                    scatter = self.create_scatter(single_asset_data, single_asset_data.carbon_custodied)
                    scatters.append(scatter)
