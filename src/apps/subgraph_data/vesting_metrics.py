...
c3_df = sg.query_df([
  c3_vesting_metrics_1_year.datetime,
...
def expand_vesting_metrics(filler_df, vesting_metric_df):
    '''
    Modify Vesting Metric chart in order to be compatible
    with Protocol Metric chart
    '''
    expanded_vesting_metric_df = filler_df.append(
      vesting_metric_df,
      ignore_index=True).sort_values(by=['vestingMetrics_datetime'])

    fill_vesting_df(expanded_vesting_metric_df)
    expanded_vesting_metric_df.set_index(
      'vestingMetrics_datetime',
      inplace=True, drop=False)

    expanded_vesting_metric_df = expanded_vesting_metric_df.groupby(
      expanded_vesting_metric_df.index).first()
    ...
    for index, row in vesting_metrics_df.iterrows():
        if row["vestingMetrics_totalAmountLocked"] == EMPTY:
            vesting_metrics_df.at[
              index, "vestingMetrics_totalAmountLocked"] = current_value
        else:
...
def subtract_vested_from_stake(staking_df, dataframes):
    for index, row in staking_df.iterrows():
        total_locked_for_date = locked_for_index(index, dataframes)
        currentStakingValue = row['protocolMetrics_staked_supply_percent']
        staking_df.at[index, 'protocolMetrics_staked_supply_percent'] = \
            currentStakingValue - total_locked_for_date


def locked_for_index(index, dataframes):
    total = 0
